...
Aplikasi menggunakan `laravel-mix` yang dikonfigurasi dalam `webpack.mix.cjs` untuk bundle aset.

**Build Pipelines**:
1.  **Main App**: `resources/js/app.js` & `resources/css/app.css` -> `public/`
2.  **Front Custom**: `style.css` -> `public/css/front-style.css`
3.  **Plugins Bundle**: Menggabungkan library eksternal (jQuery, SweetAlert2, Select2, Dropzone) ke dalam `main-plugins.js` dan `profile-plugins.js` untuk mengurangi request HTTP.
4.  **Datatables**: Bundle khusus untuk Datatables + Bootstrap 5 + Export Buttons (PDFMake, JSZip).
5.  **Slider**: Revolution Slider assets.
...
mix.scripts([
    ...
    ...
], 'public/js/main-plugins.js');
```

## Theme Architecture (`App\Core\Theme`)

Tema Admin (Metronic) dikelola secara dinamis melalui helper class `App\Core\Theme.php`. Class ini bertugas untuk:

*   **Asset Injection**: Menambahkan file CSS/JS spesifik halaman via `addVendor()` atau `addCssFile()`.
*   **Mode Switch**: Menangani Light/Dark mode dan RTL direction.
*   **Icon Helper**: Method `getIcon('name')` untuk merender SVG icon (Duotone/Outline).
*   **HTML Attributes**: Menambahkan class pada body/html tag secara dinamis.

**Contoh Penggunaan di Blade**:
```blade
<!-- master.blade.php -->
<html {!! printHtmlAttributes('html') !!}>
...
